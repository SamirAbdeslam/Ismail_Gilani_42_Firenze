...
n_tests=0
sum_moves=0
echo "" > failed_test_500.txt
echo "" > passed_test_500.txt

for ((i = 0; i < tests; i++)); do
	ARG=`seq -10000 10000 | shuf -n 500`
	moves=`.././push_swap $ARG`
	...
	n_tests=$((n_tests + 1))
	if (( $tot_moves > 5500 )); then
		echo -en "${RED}KO $tot_moves moves${RESET}"
		echo "Moves: $tot_moves" >> failed_test_500.txt
		echo $ARG >> failed_test_500.txt
		echo "" >> failed_test_500.txt
	else
		echo -en "${GREEN}OK $tot_moves moves${RESET}"
		echo "Moves: $tot_moves" >> passed_test_500.txt
		echo $ARG >> passed_test_500.txt
		echo "" >> passed_test_500.txt
	fi
	if [[ $checker == *OK* ]]; then
...
echo -e "${BLUE}Failed tests in 'failed_test_500.txt' file${RESET}"
echo -e "${BLUE}Success tests in 'passed_test_500.txt' file${RESET}"
